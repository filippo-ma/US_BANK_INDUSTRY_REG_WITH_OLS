...
def ols_reg(independent_df, dependent_df):
    ...
    for train_index, test_index in loo.split(independent):
        ...
        model_fitted = ols_model.fit(X_train, y_train.ravel())

        ols_coefs.append(model_fitted.coef_)
        ols_pred.append(np.exp(model_fitted.predict(X_test)))

    ols_coefs_df = pd.DataFrame(ols_coefs, columns=independent_df.columns, index=independent_df.index)

    return ols_coefs_df, np.array(ols_pred)
...
def desc_stat_ot(df1, df2, df3, df4, df5, df6, estimator_name, multiple_name):

    rmse_df = pd.DataFrame(np.array([
        np.sqrt((df1['valuation error']**2).mean()),
        np.sqrt((df2['valuation error']**2).mean()),
        np.sqrt((df3['valuation error']**2).mean()),
        np.sqrt((df4['valuation error']**2).mean()),
        np.sqrt((df5['valuation error']**2).mean()),
        np.sqrt((df6['valuation error']**2).mean()),
    ])).rename(columns={0:'RMSE'})

    mae_df = pd.DataFrame(np.array([
        df1['valuation error'].abs().mean(),
        df2['valuation error'].abs().mean(),
        df3['valuation error'].abs().mean(),
        df4['valuation error'].abs().mean(),
        df5['valuation error'].abs().mean(),
        df6['valuation error'].abs().mean(),   
    ])).rename(columns={0:'MAE'})

    new_df = pd.concat([rmse_df, mae_df], axis=1)

    new_df['date'] = [valuation_date1, valuation_date2, valuation_date3, valuation_date4, valuation_date5, valuation_date6]

    new_df[' '] = [
        '{0} {1}'.format(estimator_name, multiple_name), 
        '{0} {1}'.format(estimator_name, multiple_name), 
        '{0} {1}'.format(estimator_name, multiple_name), 
        '{0} {1}'.format(estimator_name, multiple_name), 
        '{0} {1}'.format(estimator_name, multiple_name), 
...
def overtime_accuracy_all_df(df_acc_ridge, df_acc_lasso, df_acc_elnet, df_acc_ols,  m_name):

    
    new_df = pd.DataFrame(pd.concat([df_acc_ridge, df_acc_lasso, df_acc_elnet, df_acc_ols]))
    ...
    ols_s = pd.Series(['ols {}'.format(m_name)]*len(df_acc_ols))
    m_names = np.array(pd.concat([ridge_s, lasso_s, elnet_s, ols_s]))
...
def overtime_accuracy_plot_ols(overtime_accuracy_df, accuracy_percentage, gen_multiple_name):
    
    fig = plt.figure(figsize=(9,4))
    sns.lineplot(data=overtime_accuracy_df, x='date', y='% of valuations within {}% of actual price'.format(accuracy_percentage), sort=False, markers=['s'], dashes=False, linewidth=1.8)
    plt.margins(y=0.25)
...
def overtime_accuracy_plot(overtime_accuracy_df, accuracy_percentage, gen_multiple_name):
    ...
    fig = plt.figure(figsize=(9,4))
    sns.lineplot(data=overtime_accuracy_df, x='date', y='% of valuations within {}% of actual price'.format(accuracy_percentage), style='valuation multiple', hue='valuation multiple', sort=False, markers=['s','s','s','s'], dashes=False, palette='viridis', linewidth=1.8)
    plt.margins(y=0.25)
    plt.title('{} accuracy over time'.format(gen_multiple_name), fontweight='bold')
    plt.tight_layout()
    plt.close()
...
def ve_descriptive_statistics(df1, df2, df4, df3, df5, df6, estimator_name, multiple_name):
...
def check_predictions_results_ols(
    ...
    ols_acc_df1 = interim_accuracy_df(ols_res_1, 'ols {0} {1}'.format(multiple_name, valuation_date1))
    ols_acc_df2 = interim_accuracy_df(ols_res_2, 'ols {0} {1}'.format(multiple_name, valuation_date2))
    ols_acc_df3 = interim_accuracy_df(ols_res_3, 'ols {0} {1}'.format(multiple_name, valuation_date3))
    ols_acc_df4 = interim_accuracy_df(ols_res_4, 'ols {0} {1}'.format(multiple_name, valuation_date4))
    ols_acc_df5 = interim_accuracy_df(ols_res_5, 'ols {0} {1}'.format(multiple_name, valuation_date5))
    ols_acc_df6 = interim_accuracy_df(ols_res_6, 'ols {0} {1}'.format(multiple_name, valuation_date6))
    ...
    ols_25_acc = accuracy_overtime_df(ols_acc_df1, ols_acc_df2, ols_acc_df3, ols_acc_df4, ols_acc_df5, ols_acc_df6, '25')
    
    
    m_25_acc = ols_25_acc
    
    # 25% accuracy plot 
    acc_plot_25 = overtime_accuracy_plot_ols(m_25_acc, '25', multiple_name)
    
    # overall accuracy 
    ols_overall_acc = overall_accuracy(ols_res_1, ols_res_2, ols_res_3, ols_res_4, ols_res_5, ols_res_6, 'ols {}'.format(multiple_name))
    
    overall_accuracy_df = ols_overall_acc

    # valuation error descriptive statistics (RMSE, MAE)
    ols_ds = ve_descriptive_statistics(ols_res_1, ols_res_2, ols_res_3, ols_res_4, ols_res_5, ols_res_6, 'ols', multiple_name)
   
    ve_desc_stat_df = ols_ds

    
    return overtime_accuracy_df, overall_accuracy_df, acc_plot_25, ve_desc_stat_df
...
def check_predictions_results(
    ...
    # acc. grouped by estimator 
    acc_df1 = pd.concat([ridge_acc_df1, lasso_acc_df1, elnet_acc_df1, ols_acc_df1])
    acc_df2 = pd.concat([ridge_acc_df2, lasso_acc_df2, elnet_acc_df2, ols_acc_df2])
    acc_df3 = pd.concat([ridge_acc_df3, lasso_acc_df3, elnet_acc_df3, ols_acc_df3])
    acc_df4 = pd.concat([ridge_acc_df4, lasso_acc_df4, elnet_acc_df4, ols_acc_df4])
    acc_df5 = pd.concat([ridge_acc_df5, lasso_acc_df5, elnet_acc_df5, ols_acc_df5])
    acc_df6 = pd.concat([ridge_acc_df6, lasso_acc_df6, elnet_acc_df6, ols_acc_df6])
    ...
    m_25_acc = overtime_accuracy_all_df(ridge_25_acc, lasso_25_acc, elnet_25_acc, ols_25_acc, multiple_name)
    ...
    ols_overall_acc = overall_accuracy(ols_res_1, ols_res_2, ols_res_3, ols_res_4, ols_res_5, ols_res_6, 'ols {}'.format(multiple_name))
    overall_accuracy_df = pd.concat([ridge_overall_acc, lasso_overall_acc, elnet_overall_acc, ols_overall_acc])

    # valuation error descriptive statistics over time 
    ridge_overtime_ds = desc_stat_ot(ridge_res_1, ridge_res_2, ridge_res_3, ridge_res_4, ridge_res_5, ridge_res_6, 'ridge', multiple_name)
    lasso_overtime_ds = desc_stat_ot(lasso_res_1, lasso_res_2, lasso_res_3, lasso_res_4, lasso_res_5, lasso_res_6, 'lasso', multiple_name)
    elnet_overtime_ds = desc_stat_ot(elnet_res_1, elnet_res_2, elnet_res_3, elnet_res_4, elnet_res_5, elnet_res_6, 'elnet', multiple_name)
    ols_overtime_ds = desc_stat_ot(ols_res_1, ols_res_2, ols_res_3, ols_res_4, ols_res_5, ols_res_6, 'ols', multiple_name)
    ve_ds_overtime_df = pd.concat([ridge_overtime_ds, lasso_overtime_ds, elnet_overtime_ds, ols_overtime_ds])
    ve_ds_overtime_df.sort_values('date', ascending=False, inplace=True)
    ...
    ols_ds = ve_descriptive_statistics(ols_res_1, ols_res_2, ols_res_3, ols_res_4, ols_res_5, ols_res_6, 'ols', multiple_name)
    ve_desc_stat_df = pd.concat([ridge_ds, lasso_ds, elnet_ds, ols_ds])

    

    
    return overtime_accuracy_df, overall_accuracy_df, acc_plot_25, ve_desc_stat_df, ve_ds_overtime_df
...
def plotting_dist_coefs(multiple_df, coefs_df, multiple_name, estimator_name):
    ...
    #plt.savefig('{0}_.png'.format(estimator_name))
    #vmax=10, vmin=-10,
...
def check_drivers_impact(
    ...
    ols_true_m1 = pd.DataFrame(ols_res1['true {}'.format(multiple_name)].rename('{}'.format(multiple_name)))
    ols_true_m2 = pd.DataFrame(ols_res2['true {}'.format(multiple_name)].rename('{}'.format(multiple_name)))
    ols_true_m3 = pd.DataFrame(ols_res3['true {}'.format(multiple_name)].rename('{}'.format(multiple_name)))
    ols_true_m4 = pd.DataFrame(ols_res4['true {}'.format(multiple_name)].rename('{}'.format(multiple_name)))
    ols_true_m5 = pd.DataFrame(ols_res5['true {}'.format(multiple_name)].rename('{}'.format(multiple_name)))
    ols_true_m6 = pd.DataFrame(ols_res6['true {}'.format(multiple_name)].rename('{}'.format(multiple_name)))
    
    
    
    ols_est_m1 = pd.DataFrame(ols_res1['estimated {}'.format(multiple_name)].rename('{}'.format(multiple_name)))
    ols_est_m2 = pd.DataFrame(ols_res2['estimated {}'.format(multiple_name)].rename('{}'.format(multiple_name)))
    ols_est_m3 = pd.DataFrame(ols_res3['estimated {}'.format(multiple_name)].rename('{}'.format(multiple_name)))
    ols_est_m4 = pd.DataFrame(ols_res4['estimated {}'.format(multiple_name)].rename('{}'.format(multiple_name)))
    ols_est_m5 = pd.DataFrame(ols_res5['estimated {}'.format(multiple_name)].rename('{}'.format(multiple_name)))
    ols_est_m6 = pd.DataFrame(ols_res6['estimated {}'.format(multiple_name)].rename('{}'.format(multiple_name)))
    
    
    ols_true_m1[' '] = pd.Series(['true']*len(ols_true_m1)).values
    ols_true_m2[' '] = pd.Series(['true']*len(ols_true_m2)).values
    ols_true_m3[' '] = pd.Series(['true']*len(ols_true_m3)).values
    ols_true_m4[' '] = pd.Series(['true']*len(ols_true_m4)).values
    ols_true_m5[' '] = pd.Series(['true']*len(ols_true_m5)).values
    ols_true_m6[' '] = pd.Series(['true']*len(ols_true_m6)).values
    
    
    ols_est_m1[' '] = pd.Series(['estimated']*len(ols_est_m1)).values
    ols_est_m2[' '] = pd.Series(['estimated']*len(ols_est_m2)).values
    ols_est_m3[' '] = pd.Series(['estimated']*len(ols_est_m3)).values
    ols_est_m4[' '] = pd.Series(['estimated']*len(ols_est_m4)).values
    ols_est_m5[' '] = pd.Series(['estimated']*len(ols_est_m5)).values
    ols_est_m6[' '] = pd.Series(['estimated']*len(ols_est_m6)).values
    
 
    
    
    ols_m1 = pd.concat([ols_true_m1, ols_est_m1])
    ols_m2 = pd.concat([ols_true_m2, ols_est_m2])
    ols_m3 = pd.concat([ols_true_m3, ols_est_m3])
    ols_m4 = pd.concat([ols_true_m4, ols_est_m4])
    ols_m5 = pd.concat([ols_true_m5, ols_est_m5])
    ols_m6 = pd.concat([ols_true_m6, ols_est_m6])
 
    
    ols_m1['date'] = pd.Series([valuation_date1]*len(ols_m1)).values
    ols_m2['date'] = pd.Series([valuation_date2]*len(ols_m2)).values
    ols_m3['date'] = pd.Series([valuation_date3]*len(ols_m3)).values
    ols_m4['date'] = pd.Series([valuation_date4]*len(ols_m4)).values
    ols_m5['date'] = pd.Series([valuation_date5]*len(ols_m5)).values
    ols_m6['date'] = pd.Series([valuation_date6]*len(ols_m6)).values
    ...
    ols_c1 = pd.DataFrame(ols_coefs1.apply(lambda x: x.median()).rename(valuation_date1))
    ols_c2 = pd.DataFrame(ols_coefs2.apply(lambda x: x.median()).rename(valuation_date2))
    ols_c3 = pd.DataFrame(ols_coefs3.apply(lambda x: x.median()).rename(valuation_date3))
    ols_c4 = pd.DataFrame(ols_coefs4.apply(lambda x: x.median()).rename(valuation_date4))
    ols_c5 = pd.DataFrame(ols_coefs5.apply(lambda x: x.median()).rename(valuation_date5))
    ols_c6 = pd.DataFrame(ols_coefs6.apply(lambda x: x.median()).rename(valuation_date6))
    
 
    
    ols_c = pd.concat([ols_c6, ols_c5, ols_c4, ols_c3, ols_c2, ols_c1], axis=1)
    ...
    ols_fig1 = plotting_dist_coefs(ols_m, ols_c, multiple_name, 'ols')
    ...
    ols_overall_coefs_df = pd.concat([ols_coefs1,
                                        ols_coefs2,
                                        ols_coefs3,
                                        ols_coefs4,
                                        ols_coefs5,
    ...
    overall_coefs_df = pd.concat([ridge_overall_coefs_df, lasso_overall_coefs_df, elnet_overall_coefs_df, ols_overall_coefs_df])
    
    overall_coefs_fig = plt.figure(figsize=(8,6))
    sns.barplot(data=overall_coefs_df, estimator=np.median, ci=95, orient='h', errwidth=1.5, order=overall_coefs_df.apply(lambda x: x.median()).sort_values(ascending=False).index.tolist(), palette='bwr')
    ...
    ols_overall_median_coefs_df = pd.DataFrame(ols_overall_coefs_df.apply(lambda x: x.median()))
    ...
    ols_name_series = pd.Series(['ols']*len(ols_overall_median_coefs_df)) 
    
    estimator_names = pd.concat([ridge_name_series, lasso_name_series, elnet_name_series, ols_name_series]).values
    ...
    overall_coefs_by_est_fig = plt.figure(figsize=(12,6))
    ...
    return ridge_fig1, lasso_fig1, elnet_fig1, ols_fig1, overall_coefs_fig, overall_coefs_by_est_fig
...
def check_drivers_impact_ols(
    ...
    # overall median coefs
    overall_coefs_df = ols_overall_coefs_df
    ...
    return ols_fig1, overall_coefs_fig
